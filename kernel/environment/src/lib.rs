...
lazy_static! {
    static ref KERNEL_DEFAULT_ENVIRONMENT: Arc<Mutex<Environment>> = Arc::new(Mutex::new(Environment::default()));
}

pub fn get_default_environment() -> Option<Arc<Mutex<Environment>>> {
    Some(KERNEL_DEFAULT_ENVIRONMENT.clone())
}
