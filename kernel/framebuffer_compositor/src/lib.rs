...
//!
//! In the next step, for every `CACHE_BLOCK_HEIGHT` rows, the compositor checks if the pixel array is already cached.
...
pub static FRAME_COMPOSITOR: Mutex<FrameCompositor> = Mutex::new(
    FrameCompositor {
        caches: BTreeMap::new()  // ← BTreeMap::new() est const, HashMap::new() non
    }
);
...
impl FrameCompositor {
    ...
    fn is_cached<P: Pixel>(&self, row_pixels: &[P], dest_coord: &Coord, width: usize) -> bool {
        match self.caches.get(dest_coord) {
            Some(cache) => {
                // Use optimized hash that samples pixels
                let current_hash = hash_pixel_slice(row_pixels, 4);
                ...
                cache.content_hash == current_hash && (cache.block.bottom_right.x - cache.block.top_left.x) as usize == width
            }
            None => false
        }
    }
    ...
    fn check_and_cache<P: Pixel>(
        ...
        // Use optimized hash that samples pixels (every 4th pixel)
        let content_hash = hash_pixel_slice(pixel_slice, 4);
        
        // Remove overlapping cache blocks (new content will overwrite them).
        ...
        let new_top    = new_cache.block.top_left.y;
        let new_bottom = new_cache.block.bottom_right.y;
        let new_left   = new_cache.block.top_left.x;
        let _new_right   = new_cache.block.bottom_right.x;

        // On calcule les coordonnées-clé potentiellement concernées
        // Les clés sont alignées sur CACHE_BLOCK_HEIGHT, donc on itère seulement sur les lignes possibles
        let mut y = (new_top / CACHE_BLOCK_HEIGHT as isize) * CACHE_BLOCK_HEIGHT as isize;
        while y <= new_bottom {
            let key = Coord::new(new_left, y);
            if let Some(existing) = self.caches.get(&key) {
                if existing.overlaps_with(&new_cache) {
                    self.caches.remove(&key);
                }
            }
            y += CACHE_BLOCK_HEIGHT as isize;
        }
...
/// Fast hash for pixel slices - samples pixels to avoid hashing everything.
fn hash_pixel_slice<P: Pixel>(pixels: &[P], sample_rate: usize) -> u64 {
    if pixels.is_empty() {
        return 0;
    }
    // Simple hash: combine sampled pixels
    let mut hash = 0xcbf29ce484222325u64;
    for (i, p) in pixels.iter().enumerate() {
        if i % sample_rate == 0 {
            // Hash a sample of the pixel data
            let p_bytes = unsafe {
