        ...
        if let Some(pte_raw) = memory::paging::get_pte_raw(vaddr_obj) {
            if memory_swap::is_swap_pte(pte_raw) {
                match memory_swap::swap_in(pte_raw) {
                    Ok(new_paddr) => {
                        let new_pte = (new_paddr.value() as u64)
                            | 0b111;
                        ...
                        warn!("[memory_swap] set_pte_raw a échoué pour 0x{:x}", vaddr_aligned);
                    }
                    Err(e) => {
                        warn!("[memory_swap] swap_in échoué pour 0x{:x}: {}", vaddr_aligned, e);
                    }
                }
            }
        }
