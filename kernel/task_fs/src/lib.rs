...
impl TaskFs {
    ...
    fn get_internal(&self, node: &str) -> Result<FileOrDir, &'static str> {
        ...
        let task_dir = TaskDir::new(dir_name, &parent_dir, id, task_ref.downgrade())?;
...
impl TaskFile {
    pub fn new(task_id: usize, taskref: WeakTaskRef) -> TaskFile {
        TaskFile {
            taskref,
            task_id,
            // Assure-toi que TASKS_DIRECTORY_PATH est bien importé ou défini
    ...
    fn generate(&self) -> String {
        ...
        let cpu = Option::<cpu::CpuId>::from(taskref.running_on_cpu.load())
            .map(|cpu| format!("{}", cpu))
            .unwrap_or_else(|| String::from("-"));

        let _pinned = Option::<cpu::CpuId>::from(taskref.pinned_core.load())
            .map(|pin| format!("{}", pin))
            .unwrap_or_else(|| String::from("-"));

        let task_type = if taskref.name.contains("idle") {
            "I" // Idle task
        } else if taskref.app_crate.is_some() {
            "A" // Application
        } else {
            "K" // Kernel task
        };  

        let runstate = format!("{:?}", taskref.runstate.load());
