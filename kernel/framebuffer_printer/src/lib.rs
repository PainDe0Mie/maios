...
/// Prints a single character at (line, column) in the text area.
#[inline]
...
/// Fills a rectangular text area with a solid color. Clamps the rectangle to framebuffer bounds.
#[inline]
pub fn fill_blank<P: Pixel>(
    ...
    let row_len = (blank.bottom_right.x - blank.top_left.x) as usize;
    let row = vec![pixel; row_len];
    let mut y = blank.top_left.y;
    while y < blank.bottom_right.y {
        if let Some(start) = framebuffer.index_of(blank.top_left + (0, y - blank.top_left.y)) {
            framebuffer.composite_buffer(&row, start);
        }
        y += 1;
    }
}

/// Returns the i-th most significant bit of `char_font` (1 or 0).
#[inline(always)]
