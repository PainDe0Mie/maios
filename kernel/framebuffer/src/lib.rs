...
impl<P: Pixel> Hash for Framebuffer<P> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        // Ne hasher que les dimensions, pas tout le buffer (trop coûteux)
        // Si besoin du contenu, utiliser un hash partiel ailleurs
...
impl<P: Pixel> Framebuffer<P> {
    ...
    /// Copies the entire content of another framebuffer into this one.
    /// Panics if the dimensions of the two framebuffers do not match.
    pub fn copy_from(&mut self, other: &Self) {
        assert_eq!(self.get_size(), other.get_size(), "Framebuffer dimensions must match for copy_from");
    ...
    /// Overwrites a pixel at the given coordinate in this framebuffer
    ...
    #[inline]
    pub fn overlaps_with(&self, coordinate: Coord, width: usize, height: usize) -> bool {
        coordinate.x < self.width as isize
