            ...
            match nvme::init_from_pci(dev) {
                Ok(Some(nvme_drive_ref)) => {
                    ...
                    let cached: StorageDeviceRef = Arc::new(Mutex::new(
                        block_cache::BlockCache::new(nvme_as_storage)
                    ));
                    match fat32::mount_and_get_root(cached, &format!("nvme{}", disk_idx)) {
                        Ok(r)  => { info!("FAT32 sur nvme{}", disk_idx); mount_disk_in_vfs(r, disk_idx); }
                        Err(e) => warn!("nvme{}: pas FAT32 ({})", disk_idx, e),
                    }
                    disk_idx += 1;
                    continue;
                }
        ...
        match storage_manager::init_device(dev) {
            Ok(Some(ctrl_ref)) => {
                ...
                for ata_ref in drives {
                    let cached: StorageDeviceRef = Arc::new(Mutex::new(
                        block_cache::BlockCache::new(ata_ref)
                    ));
                    match fat32::mount_and_get_root(cached, &format!("ata{}", disk_idx)) {
                        Ok(r)  => { info!("FAT32 sur ata{}", disk_idx); mount_disk_in_vfs(r, disk_idx); }
                        Err(e) => debug!("ata{}: pas FAT32 ({})", disk_idx, e),
                    }
                    disk_idx += 1;
                }
                continue;
