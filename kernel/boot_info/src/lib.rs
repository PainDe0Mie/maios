...
pub enum FramebufferFormat {
    ...
    ///
    /// Each color component occupies 8 bits, so a pixel is at minimum 24 bits.
    /// The size of the padding bits is `bits_per_pixel - 24`.
    RgbPixel,
...
pub trait BootInformation: 'static {
    ...
    ///
    /// A boot information structure with a length of zero is almost certainly a bug.
    fn is_empty(&self) -> bool {
        let empty = self.len() == 0;
        debug_assert!(!empty, "boot information has zero length, which is likely a bug");
        empty
    }
