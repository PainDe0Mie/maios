...
fn modules_memory_region(
    ...
    if !has_modules {
        // No modules loaded: return an empty region at address 0.
        return Ok(ReservedMemoryRegion {
            start: PhysicalAddress::new(0).ok_or("invalid zero physical address")?,
            len: 0,
        });
    }
...
impl crate::BootInformation for multiboot2::BootInformation {
    ...
    fn rsdp(&self) -> Option<PhysicalAddress> {
        // We use the physical address of the RSDP tag itself, not its signature content.
        if let Some(tag) = self.rsdp_v2_tag() {
            PhysicalAddress::new(tag as *const _ as usize - KERNEL_OFFSET)
        } else if let Some(tag) = self.rsdp_v1_tag() {
            PhysicalAddress::new(tag as *const _ as usize - KERNEL_OFFSET)
        } else {
            None
        }
    ...
    fn framebuffer_info(&self) -> Option<crate::FramebufferInfo> {
        ...
        let format = match fb_tag.buffer_type {
            ...
            multiboot2::FramebufferType::RGB { red, green, blue } => {
                // Fall back to custom pixel format instead of discarding the framebuffer.
                FramebufferFormat::CustomPixel {
                    red_bit_position:    red.position,
                    red_size_in_bits:    red.size,
                    green_bit_position:  green.position,
                    green_size_in_bits:  green.size,
                    blue_bit_position:   blue.position,
                    blue_size_in_bits:   blue.size,
                }
            }
