...
impl crate::Module for Module {
    ...
    fn start(&self) -> PhysicalAddress {
        // Find the modules memory region to compute the absolute physical address.
        // This should always exist if a Module object was constructed.
        let modules_region = self.regions
            .iter()
            .find(|region| region.kind == MODULES_MEMORY_KIND)
            .expect("BUG: Module::start(): no modules memory region found; \
                     this indicates a bootloader or boot_info bug");
        PhysicalAddress::new_canonical(modules_region.start + self.inner.offset)
    }
