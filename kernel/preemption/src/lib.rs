...
pub unsafe fn force_enable_preemption() {
    let current = PREEMPTION_COUNT.load();
    if current > 0 {
        // Soustrait la valeur actuelle pour ramener à 0
        PREEMPTION_COUNT.fetch_add(u8::wrapping_neg(current));
    }
}
