...
pub fn kstart_ap(
    ...
    let namespace = mod_mgmt::get_initial_kernel_namespace()
        .expect("kstart_ap: couldn't get initial kernel namespace")
        .clone();
    let env = environment::get_default_environment()
        .expect("kstart_ap: couldn't get default environment")
        .clone();
    let bootstrap_task = spawn::init(kernel_mmi_ref.clone(), cpu_id, this_ap_stack, namespace, env).unwrap();
