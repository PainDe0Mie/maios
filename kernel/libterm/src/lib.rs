...
impl Terminal {
    ...
    pub fn new() -> Result<Terminal, &'static str> {
        ...
        let win_w = (screen_width * 80) / 100;
        let win_h = (screen_height * 80) / 100;

        let start_x = (screen_width - win_w) / 2;
        let start_y = (screen_height - win_h) / 2;

        let window = window::Window::with_title(
            "Mai Terminal".to_string(),
            Coord::new(start_x as isize, start_y as isize),
            win_w,
            win_h,
            FONT_BACKGROUND_COLOR,
        )?;
        ...
        terminal.print_to_terminal("MaiOS - Terminal v1.0.0\n(c) 2026 MaiOS Developers\n\nSystem Ready. Type 'help' for commands.\n\n".to_string());
        Ok(terminal)
    }
