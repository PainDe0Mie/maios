//! Creation and management of virtual consoles or terminals atop Mai.
        ...
        if IGNORED_SERIAL_PORT_INPUT.load(Ordering::Relaxed) == serial_port_address as u16 {
            warn!(
				"Currently ignoring inputs on serial port {:?}. \
				 \n --> Note: QEMU is forwarding control sequences (like Ctrl+C) to Mai. To exit QEMU, press Ctrl+A then X.",
				serial_port_address,
			);
            continue;
        }
