...
fn adapt_nt_allocate_vm(
    ...
    let result = maios_syscall::dispatch(
        ...
        requested_size, protect, alloc_type, 0, 0, 0,
    );

    match result {
        Ok(base_addr) => {
            warn!("windows_syscall: SYS_ALLOC_VM returned base_addr={:#x}", base_addr);
            if base_addr == 0 {
                warn!("windows_syscall: SYS_ALLOC_VM returned null — STATUS_NO_MEMORY");
