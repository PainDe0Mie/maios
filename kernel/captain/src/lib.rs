...
fn init_swap() {
    if let Some(device) = storage_manager::storage_devices().next() {
        let swap_mb = {
            ...
            // size_in_blocks() * 512 bytes / 1MB, moins 1MB de marge
            (locked.size_in_blocks() / 2048).saturating_sub(1)
        };
        memory_swap::init(device, swap_mb);
        log::info!("Swap initialisé : {} MB", swap_mb);
    } else {
        log::warn!("Swap : aucun storage device trouvé");
    }
}
...
pub fn init(
    ...
    let kernel_namespace = mod_mgmt::get_initial_kernel_namespace()
        .ok_or("captain::init(): couldn't get initial kernel namespace")?
        .clone();
    let kernel_env = environment::get_default_environment()
        .ok_or("captain::init(): couldn't get default kernel environment")?;

    let bootstrap_task = spawn::init(kernel_mmi_ref.clone(), bsp_id, bsp_initial_stack, kernel_namespace, kernel_env)?;
