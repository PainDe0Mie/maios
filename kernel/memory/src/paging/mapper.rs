...
pub fn get_pte_raw(virtual_address: VirtualAddress) -> Option<u64> {
    use x86_64::registers::control::Cr3;
    
    let p4_frame = Cr3::read().0;
    let p4_addr = p4_frame.start_address().as_u64() as *const u64;
    
    // Index dans chaque niveau de table (9 bits chacun)
    let addr = virtual_address.value() as u64;
    let p4_idx = ((addr >> 39) & 0x1FF) as usize;
    let p3_idx = ((addr >> 30) & 0x1FF) as usize;
    let p2_idx = ((addr >> 21) & 0x1FF) as usize;
    let p1_idx = ((addr >> 12) & 0x1FF) as usize;

    unsafe {
        // Niveau 4
        let p4_entry = p4_addr.add(p4_idx).read_volatile();
        if p4_entry & 1 == 0 { return None; } // P4 non présent = pas mappé du tout

        // Niveau 3
        let p3_addr = (p4_entry & 0x000F_FFFF_FFFF_F000) as *const u64;
        let p3_entry = p3_addr.add(p3_idx).read_volatile();
        if p3_entry & 1 == 0 { return None; } // P3 non présent

        // Niveau 2
        let p2_addr = (p3_entry & 0x000F_FFFF_FFFF_F000) as *const u64;
        let p2_entry = p2_addr.add(p2_idx).read_volatile();
        if p2_entry & 1 == 0 { return None; }

        // Niveau 1 — la PTE finale
        // Ici on retourne même si present=0 !
        let p1_addr = (p2_entry & 0x000F_FFFF_FFFF_F000) as *const u64;
        let p1_entry = p1_addr.add(p1_idx).read_volatile();
        Some(p1_entry)
    }
}

pub unsafe fn set_pte_raw(virtual_address: VirtualAddress, value: u64) -> bool {
    use x86_64::registers::control::Cr3;
    ...
    let p4_entry = (p4_addr as *mut u64).add(p4_idx).read_volatile();
    if p4_entry & 1 == 0 { return false; }

    let p3_addr = (p4_entry & 0x000F_FFFF_FFFF_F000) as *mut u64;
    let p3_entry = p3_addr.add(p3_idx).read_volatile();
    if p3_entry & 1 == 0 { return false; }

    let p2_addr = (p3_entry & 0x000F_FFFF_FFFF_F000) as *mut u64;
    let p2_entry = p2_addr.add(p2_idx).read_volatile();
    if p2_entry & 1 == 0 { return false; }

    let p1_addr = (p2_entry & 0x000F_FFFF_FFFF_F000) as *mut u64;
    p1_addr.add(p1_idx).write_volatile(value);
    true
}