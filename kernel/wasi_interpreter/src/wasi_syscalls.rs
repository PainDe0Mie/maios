...
/// Write a wasi::Filestat struct into WASM memory at ptr (64 bytes).
fn write_filestat(memory: &MemoryRef, ptr: u32, fod: &FileOrDir) {
    let mut buf = [0u8; 64];
    let (filetype, size) = match fod {
        FileOrDir::Dir(_) => (wasi::FILETYPE_DIRECTORY, 0u64),
        FileOrDir::File(f) => (wasi::FILETYPE_REGULAR_FILE, f.lock().len() as u64),
    };
    buf[16] = filetype;
...
pub fn execute_system_call(
    ...
    match system_call {
        ...
        SystemCall::RandomGet => {
            let buf_ptr: u32 = wasmi_args.nth_checked(0).unwrap();
            let buf_len: wasi::Size = {
                let l: u32 = wasmi_args.nth_checked(1).unwrap();
                wasi::Size::try_from(l).unwrap()
            };
            let mut buf = vec![0u8; buf_len];
            for (i, byte) in buf.iter_mut().enumerate() {
                let tsc = unsafe { core::arch::x86_64::_rdtsc() };
                *byte = ((tsc >> ((i % 8) * 8)) ^ (i as u64).wrapping_mul(0x9E3779B97F4A7C15)) as u8;
            }
            memory.set(buf_ptr, &buf).unwrap();
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
        }

        SystemCall::FdFilestatGet => {
            let posix_node: &mut PosixNode = {
                let fd: wasi::Fd = wasmi_args.nth_checked(0).unwrap();
                match fd_table.get_posix_node(fd) {
                    Some(pn) => pn,
                    None => return Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_BADF)))),
                }
            };
            let filestat_ptr: u32 = wasmi_args.nth_checked(1).unwrap();
            write_filestat(memory, filestat_ptr, &posix_node.theseus_file_or_dir);
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
        }

        SystemCall::PathFilestatGet => {
            let parent_dir: DirRef = {
                let fd: wasi::Fd = wasmi_args.nth_checked(0).unwrap();
                let posix_node = match fd_table.get_posix_node(fd) {
                    Some(pn) => pn,
                    None => return Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_BADF)))),
                };
                match posix_node.theseus_file_or_dir.clone() {
                    FileOrDir::Dir(d) => d,
                    FileOrDir::File { .. } => return Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOTDIR)))),
                }
            };
            let path_str = {
                let ptr: u32 = wasmi_args.nth_checked(2).unwrap();
                let len: wasi::Size = wasi::Size::try_from(wasmi_args.nth_checked::<u32>(3).unwrap()).unwrap();
                String::from_utf8(memory.get(ptr, len).unwrap()).unwrap()
            };
            let filestat_ptr: u32 = wasmi_args.nth_checked(4).unwrap();
            let p = path::Path::new(&path_str);
            match p.get(&parent_dir) {
                Some(fod) => {
                    write_filestat(memory, filestat_ptr, &fod);
                    Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
                }
                None => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOENT)))),
            }
        }

        SystemCall::FdReaddir => {
            let dir_ref: DirRef = {
                let fd: wasi::Fd = wasmi_args.nth_checked(0).unwrap();
                let posix_node = match fd_table.get_posix_node(fd) {
                    Some(pn) => pn,
            ...
            let buf_ptr: u32  = wasmi_args.nth_checked(1).unwrap();
            let buf_len: wasi::Size = wasi::Size::try_from(wasmi_args.nth_checked::<u32>(2).unwrap()).unwrap();
            let cookie: wasi::Dircookie = wasmi_args.nth_checked(3).unwrap();
            let buf_used_ptr: u32 = wasmi_args.nth_checked(4).unwrap();

            let entries = dir_ref.lock().list();
            let mut buf_written: usize = 0;
            let mut write_pos: u32 = buf_ptr;

            for (i, name) in entries.iter().enumerate().skip(cookie as usize) {
                let name_bytes = name.as_bytes();
                let dirent_size = 24 + name_bytes.len();
                if buf_written + dirent_size > buf_len { break; }

                let next_cookie: u64 = (i as u64) + 1;
                memory.set(write_pos, &next_cookie.to_le_bytes()).unwrap();
                memory.set(write_pos + 8, &(i as u64).to_le_bytes()).unwrap();
                memory.set(write_pos + 16, &(name_bytes.len() as u32).to_le_bytes()).unwrap();
                let filetype: u8 = match dir_ref.lock().get(name) {
                    Some(FileOrDir::Dir(_))  => wasi::FILETYPE_DIRECTORY,
                    Some(FileOrDir::File(_)) => wasi::FILETYPE_REGULAR_FILE,
                    None => wasi::FILETYPE_UNKNOWN,
                };
                memory.set(write_pos + 20, &[filetype, 0, 0, 0]).unwrap();
                memory.set(write_pos + 24, name_bytes).unwrap();

                write_pos += dirent_size as u32;
                buf_written += dirent_size;
            }
            memory.set(buf_used_ptr, &(buf_written as u32).to_le_bytes()).unwrap();
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
        }

        SystemCall::PathCreateDirectory => {
            let parent_dir: DirRef = {
                let fd: wasi::Fd = wasmi_args.nth_checked(0).unwrap();
                let posix_node = match fd_table.get_posix_node(fd) {
                    Some(pn) => pn,
            ...
            let dir_name = {
                let ptr: u32 = wasmi_args.nth_checked(1).unwrap();
                let len = wasi::Size::try_from(wasmi_args.nth_checked::<u32>(2).unwrap()).unwrap();
                String::from_utf8(memory.get(ptr, len).unwrap()).unwrap()
            };
            match vfs_node::VFSDirectory::create(dir_name, &parent_dir) {
                Ok(_)  => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS)))),
                Err(_) => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_IO)))),
            }
        }

        SystemCall::PathRemoveDirectory => {
            let parent_dir: DirRef = {
                let fd: wasi::Fd = wasmi_args.nth_checked(0).unwrap();
                let posix_node = match fd_table.get_posix_node(fd) {
                    Some(pn) => pn,
            ...
            let p = path::Path::new(&path_str);
            match p.get_dir(&parent_dir) {
                Some(dir) => {
                    let fod = FileOrDir::Dir(dir);
                    match parent_dir.lock().remove(&fod) {
                        Some(_) => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS)))),
                        None    => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_IO)))),
                    }
                }
                None => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOENT)))),
            }
        ...
        SystemCall::PathUnlinkFile => {
            ...
            let p = path::Path::new(&path_str);
            match p.get_file(&parent_dir) {
                Some(file) => {
                    let fod = FileOrDir::File(file);
                    match parent_dir.lock().remove(&fod) {
                        Some(_) => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS)))),
                        None    => Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_IO)))),
                    }
        ...
        SystemCall::PathRename => {
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOTSUP))))
        }

        SystemCall::FdSync | SystemCall::FdDatasync => {
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
        }

        // ── BATCH 2 ─────────────────────────────────────────────────────────

        SystemCall::PathSymlink | SystemCall::PathReadlink | SystemCall::PathLink => {
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOTSUP))))
        }

        SystemCall::FdFilestatSetSize => {
            let _fd: wasi::Fd = wasmi_args.nth_checked(0).unwrap();
            let size: wasi::Filesize = wasmi_args.nth_checked(1).unwrap();
            if size == 0 { Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS)))) }
            else         { Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOTSUP)))) }
        }

        SystemCall::FdFilestatSetTimes | SystemCall::PathFilestatSetTimes => {
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
        }
        ...
        SystemCall::ProcRaise => {
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOTSUP))))
        }

        SystemCall::SockAccept | SystemCall::SockRecv | SystemCall::SockSend | SystemCall::SockShutdown => {
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_NOTSUP))))
        }

        SystemCall::FdAdvise | SystemCall::FdAllocate => {
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
        }
        ...
        SystemCall::FdTell => {
            ...
            let offset_ptr: u32 = wasmi_args.nth_checked(1).unwrap();
            // seek(0, WHENCE_CUR) = ftell
            let pos: u64 = match posix_node.seek(0, wasi::WHENCE_CUR) {
                Ok(p)  => p as u64,
                Err(_) => 0,
            };
            memory.set(offset_ptr, &pos.to_le_bytes()).unwrap();
            Ok(Some(RuntimeValue::I32(From::from(wasi::ERRNO_SUCCESS))))
        }
    }
}