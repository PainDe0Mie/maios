...
const GLYPH_WIDTH: u32 = CHARACTER_WIDTH - 1;
...
pub fn init(
    ...
    // Use the current pixel coordinate if the early_printer has already been initialized,
...
pub fn take() -> Option<EarlyFramebufferPrinter> {
    EARLY_FRAMEBUFFER_PRINTER.lock().take().and_then(|ep| {
        if let EarlyPrinter::Framebuffer(early_fb) = ep {
            Some(early_fb)
        } else {
            None
        }
...
/// Default foreground color used when printing via the `Write` trait.
const DEFAULT_FOREGROUND: u32 = 0x0000FF; // Blue
/// Default background color used when printing via the `Write` trait.
const DEFAULT_BACKGROUND: u32 = 0xD3D3D3; // Light gray
...
impl EarlyFramebufferPrinter {
    ...
    pub fn print_char(
        ...
        for (row_bits, row) in glyph.iter().zip(0u32..) {
            ...
            for (pixel, col) in pixel_row.iter_mut().zip(0 .. GLYPH_WIDTH) {
            ...
            // Always write to the main fb so the character is immediately visible.
            self.fb[fb_row_range.clone()].copy_from_slice(&pixel_row);
            // Also keep the staging fb in sync (used to accelerate scrolling).
            if let Some(staging) = self.staging_fb.as_deref_mut() {
                staging[fb_row_range].copy_from_slice(&pixel_row);
            }
        }
...
impl Write for EarlyFramebufferPrinter {
    fn write_str(&mut self, s: &str) -> fmt::Result {
        for ch in s.chars() {
            self.print_char(ch, DEFAULT_FOREGROUND, DEFAULT_BACKGROUND);
        }
        Ok(())
    }
}
