...
use core::{fmt::{self, Write}, ptr::NonNull};
...
pub struct VgaBuffer {
    ...
    buffer: NonNull<VgaTextBuffer>,
}

// SAFETY: `VgaBuffer` wraps a raw pointer to a fixed, identity-mapped hardware address.
// Access is always serialized through the global `Mutex<Option<EarlyPrinter>>`.
...
impl VgaBuffer {
    pub const fn new() -> Self {
        VgaBuffer {
            column_position: 0,
            // SAFETY: VGA_BUFFER_VIRTUAL_ADDR is a non-null, identity-mapped hardware address.
            buffer: unsafe { NonNull::new_unchecked(VGA_BUFFER_VIRTUAL_ADDR as *mut _) },
        }
    }
    ...
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            ...
            // Only write printable ASCII; replace anything else with '■' (0xfe in VGA font).
            byte => {
                let byte = if byte.is_ascii_graphic() || byte == b' ' { byte } else { 0xfe };
...
impl Write for VgaBuffer {
    fn write_str(&mut self, s: &str) -> fmt::Result {
        for ch in s.chars() {
            // Map non-ASCII chars to '?' — the VGA font only covers ASCII.
