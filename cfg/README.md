## MaiOS target specifications

MaiOS uses its own target specification to configure how `rustc` compiles its code.
The default target spec is [x86_64-unknown-theseus.json], which builds MaiOS for the `x86_64` architecture.
...
* `unknown`: the vendor.
  * MaiOS doesn't have a specific vendor; the default value is `unknown`.
* `theseus`: the operating system identifier used by the Rust target.
  * Optionally, the OS parameter can be appended with other items, such as the system environment or ABI.
    * MaiOS doesn't yet specify an environment or ABI,
...
* `llvm-target`: all MaiOS targets are based on `x86_64-unknown-none-elf`,
  which is a minimal target that specifies no underlying OS and
  uses the ELF file format for compiled artifacts.
* `features`: `-mmx,-sse,+soft-float`.
  This builds MaiOS with hardware floating point support disabled in favor of soft floating point.
...
* [`code-model`]: we use the `large` code model because MaiOS runs in a single address space,
...
* [`tls-model`]: we use the `local-exec` model for Thread-Local Storage (TLS) because
  ...
  Also, MaiOS's runtime loader/linker can even support the `local-exec` TLS model
  ...
  * Currently, MaiOS doesn't support the other three TLS models, as some of them
...
* [`merge-functions`]: we disable this option in order to ensure that `loadable` mode
  works correctly, in which MaiOS loads and links all crate object files at runtime.
  ...
  * It appears that MaiOS still works correctly without setting this option,
...
* [x86_64-unknown-theseus-sse.json]: similar to the default `x86_64-unknown-theseus`, but enables the compiler to
  ...
  With this, all crates across MaiOS can use SSE2 SIMD instructions and registers.
* [x86_64-unknown-theseus-avx.json]: similar to above, but enables AVX (version 1) instructions/registers.
  With this, all crates across MaiOS can use AVX SIMD instructions and registers.
