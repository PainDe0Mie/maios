...
pub fn main(_: Vec<String>) -> isize {
    task::set_kill_handler(Box::new(|_task, _kill_reason| {
        QEMU_EXIT_HANDLE.exit_failure();
    }))
    .unwrap();

    let dir = task::get_my_current_task()
        .map(|t| t.namespace.dir().clone())
...
pub fn run_test(path: &Path) -> Result<(), ()> {
    match spawn::new_application_task_builder(path, None)
        ...
        .unwrap()
    {
        ExitValue::Completed(status) => if status == 0 { Ok(()) } else { Err(()) },
        ExitValue::Killed(KillReason::Requested) => unreachable!(),
        ExitValue::Killed(KillReason::Panic) => Err(()),
