            ...
            if let Some(task_refs) = self.jobs.get(&fg_job_num).map(|job| &job.tasks) {
                ...
                for task_ref in task_refs {
                    ...
                    match task_ref.kill(KillReason::Requested) {
                        Ok(_) => { // task_ref here is JoinableTaskRef, so kill() is valid
                            remove_task(task_ref); // Use the imported remove_task
                        }
...
impl Shell {
    ...
    fn execute_internal(&mut self) -> Result<(), &'static str> {
        ...
        if let Some(cmd) = iter.next() {
            match cmd {
                ...
                "ps" | "tasks" | "tasklist" => self.execute_internal_tasks(),
                "fg" => self.execute_internal_fg(),
                "bg" => self.execute_internal_bg(),
                "clear" | "cls" => self.execute_internal_clear(),
    ...
    fn execute_internal_tasks(&mut self) -> Result<(), &'static str> {
        ...
        for (cpu, task_list) in all_tasks {
            for t in task_list {
                total_tasks += 1;
                term.print_to_terminal(format!("{:<6} | {:<5?} | {}\n", t.id, cpu, t.name));
            }
...
fn shell_loop(_dummy: ()) -> Result<(), &'static str> {
    info!("shell_loop started on CPU {}", cpu::current_cpu());
    let shell = Shell::new().map_err(|e| {
        error!("Shell::new failed: {}", e);
        e
    })?;
    info!("Shell created, starting main loop");
    shell.start()?;
    Ok(())
}
